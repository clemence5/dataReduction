...
################ XII: Selfcal2, shorter solint = 120s, 3c84 only ########################
for i, j in enumerate(FL):
...
###########################################################################################
################ XIII: Deconvolve, 3c84 only thr = 0.8mJy ########################

cleanname= '3c84_only_subbed_120s' #_round2_60s' #_30s_acal'
...
'''
###########################################################################################
################ XIV: Selfcal2, shorter solint = 60s, 3c84 only ########################
for i, j in enumerate(FL):
...
###########################################################################################
################ XV: Deconvolve, 3c84 only thre = 0.75mJy ########################
...
###########################################################################################
################ XVI: Selfcal2, shorter solint = 60s, 3c84 only ########################
for i, j in enumerate(FL):
    ...
    calt = 'caltable_'+str(freq)+'_60s_apcal.G0'       #calibration table
    print "self calibrating 3c84-north.%s.MS" %freq
    gaincal(vis=j, gaintype='G', solint='60s',combine='scan', caltable=calt, calmode='ap') #amp+phase gain calibration
    print "applying amp and phase gain calibration only!"
    applycal(vis=j, gaintable= calt)   #apply gain calibration  | plot result
    plotcal(caltable=calt, xaxis='time', yaxis='phase', poln='X', subplot =221, iteration = 'antenna', figfile='selfcal_'+str(freq)+'_phaseVtime_60s_apcal.png', showgui=False)
...
cleanname= '3c84_only_subbed_60s_ampcal' #_round2_60s' #_30s_acal'
